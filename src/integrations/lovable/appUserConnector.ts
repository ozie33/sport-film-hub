 ...
 *
 * SERVER-ONLY. Read by TanStack Start server functions / server routes to
 * start per-user OAuth and to call provider APIs on behalf of the app user
 * who completed consent. Importing this into a browser bundle would leak
 * workspace credentials, so it is only ever imported from handlers.
 */
...
export interface AppUserOAuthAuthorizeParams {
  ...
  clientAPIKey: string;
  returnUrl: string;
  /** Present on reconnect so the gateway can confirm ownership. */
...
export interface CallAsAppUserParams {
  ...
  connectionAPIKey: string;
  connectorId: string;
  /** Path under the connector, e.g. "/drive/v3/files". Leading slash optional. */
  path: string;
  init?: RequestInit;
}
